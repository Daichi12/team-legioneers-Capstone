...
// Parse opts from properties.
// https://github.com/scottwrobinson/camo
// Currently the supported options/validators are:
const currentlySupported = ['type', 'default', 'min', 'max', 'choices', 'match', 'validate', 'unique', 'required']
const customOpts = (prop) => { // title: {type:String, default:'', isHtml:true}
  const custom = Object.assign({}, prop)
  currentlySupported.forEach(key => {
    delete custom[key]
  })

  const keys = Object.keys(custom)
  return (keys.length == 0) ? null : custom
}
...
class BaseDocument {
     ...
     */
    schema(extension) {
        if (extension.opts == null){
          const opts = {}
          const keys = Object.keys(extension)
          keys.forEach(key => {
            const prop = extension[key] // title: {type:String, default:'', isHtml:true}
            const custom = customOpts(prop)
            if (custom != null){
              opts[key] = custom
